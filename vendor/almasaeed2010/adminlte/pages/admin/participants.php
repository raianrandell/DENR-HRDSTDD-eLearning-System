...
$(document).ready(function () {
    var table = $("#traineesTable").DataTable({
        ...
        "autoWidth": false,
        "columnDefs": [
            { "targets": [1, 5, 6, 7], "className": "text-nowrap" }
        ]
    });
    ...
    $("#trainingFilter").on("change", function () {
        ...
        table.column(5).search(selectedValue).draw(); // Column 5 (In Training)
    });

    // Reset filter
    $("#resetFilter").on("click", function () {
        $("#trainingFilter").val(""); // Reset dropdown
        table.column(5).search("").draw(); // Reset search
    });
});
  ...
  // Use event delegation to handle dynamically loaded rows
