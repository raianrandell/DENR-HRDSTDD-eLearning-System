...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $sql = "UPDATE training SET description=?, location=?, start_date=?, end_date=?, training_hrs=? WHERE training_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssii", $description, $location, $start_date, $end_date, $training_hrs, $training_id);
        $stmt->execute();
